...
class DefaultConfigs(ABC):
    ...
    arch = "resnet50"  # Arquitetura do modelo
    datasets = ["zhaolian_train"]  # Conjunto de dados de treinamento
    datasets_test = ["imagensPara-Treinamento"]  # Conjunto de dados de teste
    mode = "binary"  # Modo do modelo
    class_bal = False  # Balanceamento de classes
    batch_size = 64  # Tamanho do lote
    loadSize = 256  # Tamanho de carregamento da imagem
    cropSize = 224  # Tamanho do corte da imagem
    epoch = "latest"  # Época de início do treinamento
    num_workers = 20  # Número de workers para carregar os dados
    serial_batches = False  # Amostragem serial dos lotes
    isTrain = True  # Modo de treinamento
    ...
    ####### train setting ######
    warmup = False  # Ativar etapa de aquecimento
    warmup_epoch = 3  # Número de épocas de aquecimento
    earlystop = True  # Ativar parada antecipada
    earlystop_epoch = 5  # Época para ativar a parada antecipada
    optim = "adamw"  # Algoritmo de otimização
    new_optim = False  # Usar otimizador novo
    loss_freq = 400  # Frequência para exibir a perda
    save_latest_freq = 2000  # Frequência para salvar o último modelo
    save_epoch_freq = 20  # Frequência para salvar o modelo a cada época
    continue_train = False  # Continuar o treinamento de um modelo salvo
    epoch_count = 1  # Contagem de épocas
    last_epoch = -1  # Última época treinada
    nepoch = 400  # Número total de épocas
    beta1 = 0.9  # Beta1 para o otimizador
    lr = 0.0001  # Taxa de aprendizado inicial
    init_type = "normal"  # Tipo de inicialização de pesos
    init_gain = 0.02  # Ganho para a inicialização
...
parser.add_argument("--ckpt", default="lsun_adm.pth", type=str)
